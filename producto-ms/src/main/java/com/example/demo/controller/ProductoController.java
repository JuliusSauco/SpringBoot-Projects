...
public class ProductoController {
	...
	@GetMapping("/tipos")
	public List<TipoProductoDTO> obtenerTiposDeProducto() {
		ModelMapper mapper = new ModelMapper();
		Iterable<TipoProducto> tipos = tipoProductoService.obtenerTiposDePorductos();
		return StreamSupport.stream(tipos.spliterator(), false).map(p -> mapper.map(p, TipoProductoDTO.class))
				.collect(Collectors.toList());
	}
